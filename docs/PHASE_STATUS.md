...
- 新增版本化 Catalog 契约、五种 `deepwrite.json` 项目清单，以及项目新建 / 打开 / 解除注册 / 整目录删除、条目创建 / 保存 / 删除等类型安全语义命令
...
- 新书在当前工作目录的 `books/` 中创建项目文件夹、根 manifest 和六个短篇阶段 Markdown；旧版 ZIP 导入会映射六个当前阶段并保留其他非空文稿；书籍改名和绑定更新写 manifest，并分别支持保留文件夹的“移除”和连同项目文件夹删除的“删除”
- 素材库、技能库及两类分组同样迁移为独立文件夹；素材与技能库支持新建 / 打开，以及与书籍统一样式的“移除 / 删除”，条目支持创建、右侧显式保存和风险确认后删除对应 Markdown
